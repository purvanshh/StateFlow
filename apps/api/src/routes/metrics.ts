...
metricsRouter.get('/prometheus', async (_req: Request, res: Response) => {
  try {
    ...
    output += `stateflow_executions_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.EXECUTIONS_TOTAL])}\n`;
    output += `# TYPE stateflow_executions_success_total counter\n`;
    output += `stateflow_executions_success_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.EXECUTIONS_SUCCESS])}\n`;
    output += `# TYPE stateflow_executions_failed_total counter\n`;
    output += `stateflow_executions_failed_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.EXECUTIONS_FAILED])}\n`;
    output += `# TYPE stateflow_executions_cancelled_total counter\n`;
    output += `stateflow_executions_cancelled_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.EXECUTIONS_CANCELLED])}\n`;
    output += `# TYPE stateflow_steps_total counter\n`;
    output += `stateflow_steps_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.STEPS_TOTAL])}\n`;
    output += `# TYPE stateflow_dlq_total counter\n`;
    output += `stateflow_dlq_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.DLQ_TOTAL])}\n`;
    output += `# TYPE stateflow_timeouts_total counter\n`;
    output += `stateflow_timeouts_total ${formatValue(allMetrics.counters?.[METRIC_NAMES.TIMEOUTS_TOTAL])}\n\n`;
