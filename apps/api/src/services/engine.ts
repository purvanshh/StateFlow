...
export async function runWorkflowExecution(executionId: string): Promise<void> {
  ...
  try {
    while (currentStepId) {
      ...
      if (result.status === 'completed') {
        ...
        if (attempts < maxAttempts) {
          // Note: If attempts < max, we schedule retry. If attempts == max, next one is fatal? No, attempts counts current failure.
    ...
    metrics.increment(METRIC_NAMES.EXECUTIONS_SUCCESS);
    metrics.observe(METRIC_NAMES.EXECUTION_DURATION_MS, duration);
    metrics.set(
      METRIC_NAMES.ACTIVE_EXECUTIONS,
      Math.max(0, (metrics.getGauge(METRIC_NAMES.ACTIVE_EXECUTIONS) || 1) - 1)
    );
